...
import { DEITIES, STATES_INDIA, DELIVERY_OPTIONS } from '../constants';
import { FormInput, FormSelect, FormTextarea, DynamicList, ImagePreview } from './SharedUI';

interface ChadhavaFormProps {
  chadhava: ChadhavaProduct | null;
  defaultReceives: string[];
  defaultDelivery: string;
  onSave: (c: ChadhavaProduct) => void;
  onClose: () => void;
  showToast: (msg: string, type: 'success' | 'error') => void;
}

const emptyChadhava = (defaults: string[], delivery: string): ChadhavaProduct => ({
  id: 0, temple: '', city: '', state: '', deity: '', offering: '', price: 0,
  status: 'inactive', shortDescription: '', significance: '',
  devoteeReceives: [...defaults],
  primaryImage: '', additionalImages: ['', '', ''], altText: '',
  videoDelivery: delivery, certificateIncluded: true, nameGotra: true,
  lastModified: new Date().toISOString(),
});

export const ChadhavaForm = ({ chadhava, defaultReceives, defaultDelivery, onSave, onClose, showToast }: ChadhavaFormProps) => {
  const isEdit = chadhava !== null;
  const [form, setForm] = useState<ChadhavaProduct>(
    chadhava ? { ...chadhava, additionalImages: [...(chadhava.additionalImages || []), '', '', ''].slice(0, 3) } : emptyChadhava(defaultReceives, defaultDelivery)
  );
  const [tab, setTab] = useState(0);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [dirty, setDirty] = useState(false);

  const update = useCallback(<K extends keyof ChadhavaProduct>(key: K, val: ChadhavaProduct[K]) => {
  ...
  const validate = useCallback(() => {
    ...
    if (!form.temple.trim()) e.temple = 'Required';
    if (!form.city.trim()) e.city = 'Required';
    if (!form.state) e.state = 'Required';
    if (!form.deity) e.deity = 'Required';
    if (!form.offering.trim()) e.offering = 'Required';
  ...
  const handleSave = useCallback((status: 'active' | 'inactive') => {
    ...
    const saved: ChadhavaProduct = {
      ...form, status,
      lastModified: new Date().toISOString(),
      additionalImages: form.additionalImages.filter(u => u.trim()),
    };
    onSave(saved);
    showToast(`${saved.temple} saved successfully`, 'success');
    ...
    <div className="fixed inset-0 z-[80] flex justify-end bg-black/40 backdrop-blur-sm" onClick={handleBackdropClick}>
      <div className="flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between border-b border-amber-100 px-6 py-4">
          <h3 className="text-lg font-bold text-stone-800">{isEdit ? 'Edit Chadhava' : 'Add New Chadhava'}</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {tab === 0 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <FormInput label="Temple Name" value={form.temple} onChange={v => update('temple', v)} required error={errors.temple} placeholder="e.g., Kashi Vishwanath" />
                <FormInput label="Temple City" value={form.city} onChange={v => update('city', v)} required error={errors.city} placeholder="e.g., Varanasi" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <FormSelect label="Temple State" value={form.state} onChange={v => update('state', v)} options={STATES_INDIA} required error={errors.state} />
                <FormSelect label="Associated Deity" value={form.deity} onChange={v => update('deity', v)} options={DEITIES} required error={errors.deity} />
              </div>
              <FormInput label="Offering Items" value={form.offering} onChange={v => update('offering', v)} required error={errors.offering} placeholder="e.g., Bel Patra & Dhatura" />
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="mt-1.5 flex gap-4">
                  {(['active', 'inactive'] as const).map(s => (
                    <label key={s} className="flex cursor-pointer items-center gap-2 text-sm text-stone-700">
                      <input type="radio" name="chadhava-status" checked={form.status === s} onChange={() => update('status', s)} className="accent-orange-600" />
          ...
          {tab === 1 && (
            <div className="space-y-5">
              <FormTextarea label="Short Description" value={form.shortDescription} onChange={v => update('shortDescription', v)} placeholder="Brief description of the offering" />
              <FormTextarea label="Significance of this offering" value={form.significance} onChange={v => update('significance', v)} placeholder="Why is this offering sacred?" />
              <DynamicList label="What Devotee Receives" items={form.devoteeReceives} onChange={v => update('devoteeReceives', v)} />
            </div>
          )}
          ...
          {tab === 3 && (
            <div className="space-y-4">
              ...
              <label className="flex cursor-pointer items-center gap-2 text-sm text-stone-700">
                <input type="checkbox" checked={form.certificateIncluded} onChange={e => update('certificateIncluded', e.target.checked)} className="rounded accent-orange-600" />
