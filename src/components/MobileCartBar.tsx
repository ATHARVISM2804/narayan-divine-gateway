...
/**
 * Floating cart bar shown ONLY on mobile devices (md:hidden).
...
const MobileCartBar = () => {
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 md:hidden animate-fadeIn">
        ...
        <Link to="/cart" className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="relative grid h-11 w-11 place-items-center rounded-full bg-saffron text-white shadow-md">
              ...
              <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-gold text-[10px] font-bold text-maroon shadow-sm">
                {totalItems}
              </span>
            </div>
            <div>
          ...
          <span className="rounded-full bg-saffron px-5 py-2 text-sm font-bold text-white shadow-md transition-all hover:bg-gold hover:text-maroon">
            View Cart →
          </span>
        </Link>
      </div>
