...
const Navbar = () => {
  ...
  const LangToggle = ({ mobile = false }: { mobile?: boolean }) => (
    <div className={`flex items-center gap-1 rounded-full border border-gold/40 bg-white/10 p-0.5 ${mobile ? "mx-3 mt-2" : ""}`}>
              ...
              {user ? (
                <>
                  <button onClick={() => { handleLogout(); setOpen(false); }}
                    className="flex-1 rounded-full border border-gold px-4 py-2 text-sm text-maroon text-center">{t("nav_logout")}</button>
                  <NavLink to="/puja" onClick={() => setOpen(false)}
                    className="flex-1 rounded-full bg-saffron px-4 py-2 text-sm font-semibold text-white text-center">{t("nav_book_now")}</NavLink>
                </>
              ) : (
                <>
                  <NavLink to="/login" onClick={() => setOpen(false)} className="flex-1 rounded-full border border-gold px-4 py-2 text-sm text-maroon text-center">{t("nav_login")}</NavLink>
                  <NavLink to="/puja" onClick={() => setOpen(false)} className="flex-1 rounded-full bg-saffron px-4 py-2 text-sm font-semibold text-white text-center">{t("nav_book_now")}</NavLink>
                </>
              )}
            </li>
          </ul>
